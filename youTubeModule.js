...
function lookupCallback (data, videoResults) {
  ...
  for (var i = 0; i < videoResults.length; i++) {
    data.runTime.videoOptions.push({url: videoResults[i].url, title: videoResults[i].title});
    data.bot.say(data.runTime.channel, (i + 1) + ": " + videoResults[i].title);
  }
}
...
exports.getByNumber = function (data, args) 
{
  if (args[0] > data.runTime.videoOptions.length || args[0] < 1) {
    data.bot.say(data.runTime.channel, "Sorry, but there is no video listed under that number...");
  }
